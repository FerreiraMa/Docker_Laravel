...
@section('content')
    <h1 class="text-center text-3x1 uppercase font-black my-4">Detalhes do {{$post->title}}</h1>

    <div class="w-11/12 p-12 bg-white sm:w-8/12 md:w-1/2 lg:w-5/12 mx-auto">
    <ul>
        <li><strong>Título: </strong> {{$post->title}}</li>
        <li><strong>Imagem: </strong> <img src="{{ url("storage/{$post->image}") }}" alt="{{ $post->title }}" class="<w-25>"></li>
